...
# =========================================================
# PANEL (UI) YARDIMCI ÇİZİMLER
# =========================================================
def draw_alpha_rect(img, x1, y1, x2, y2, color_bgr=(0, 0, 0), alpha=0.4, radius=0):
    """
    Basit alpha rect. (radius şu an kullanılmıyor; stabil kalsın diye)
    """
...
def draw_car_icon(img, x, y, scale=1.0, color=(255, 255, 255)):
    """
    Küçük "arkadan araç" ikon çizimi (emoji değil; her ortamda stabil).
    x,y: ikonun sol-üst köşesi
    """
    x = int(x); y = int(y)
    s = float(scale)
    ...
    bw = int(26 * s)
    bh = int(14 * s)
    cv2.rectangle(img, (x, y + int(6*s)), (x + bw, y + int(6*s) + bh), color, -1)
    ...
    cx1 = x + int((bw - cw) / 2)
    cy1 = y + int(2 * s)
    cv2.rectangle(img, (cx1, cy1), (cx1 + cw, cy1 + ch), color, -1)

    # Stop lambaları (kırmızımsı)
    lx = x + int(3*s); rx = x + bw - int(5*s)
    ly = y + int(10*s)
    cv2.rectangle(img, (lx, ly), (lx + int(2*s), ly + int(4*s)), (0, 0, 255), -1)
    cv2.rectangle(img, (rx, ly), (rx + int(2*s), ly + int(4*s)), (0, 0, 255), -1)

    # Tampon çizgisi
    cv2.line(img, (x + int(2*s), y + int(6*s) + bh), (x + bw - int(2*s), y + int(6*s) + bh), (0, 0, 0), 1)
...
while True:
            ...
            if spot_index != -1:
                ...
                if final_id == VIP_ID:
                    ...
                    if final_id not in GLOBAL_PARKED_REGISTRY:
                        # ÖNEMLİ: artık süre göstereceğiz -> start_time set et
                        GLOBAL_PARKED_REGISTRY[final_id] = {"plate": park_plate, "start_time": now_wall}
                    if park_plate != "[...]":
                        GLOBAL_PARKED_REGISTRY[final_id]["plate"] = park_plate
                    # start_time hiç set edilmediyse (eski kayıtlardan kalırsa) tamamla
                    if GLOBAL_PARKED_REGISTRY[final_id].get("start_time") is None:
                        GLOBAL_PARKED_REGISTRY[final_id]["start_time"] = now_wall
            else:
    ...
    # =====================================================
    # ÇİZİM / PANEL (YENİ TASARIM)
    # - SAĞ ALT OKUNAN PLAKA KALDIRILDI
    # - SOL ALT PARK HALİNDE: başlık beyaz, açık şeffaf panel
    # - Satır: araç ikon | ID no | plaka | süre | yeşil yuvarlak
    # - Satırların arka planı panelden daha koyu şerit
    # =====================================================
    # Panel ölçüleri
    panel_w = 520
    panel_h = 260
    ...
    # Panel arka planı: daha açık şeffaf (eskiye göre)
    draw_alpha_rect(img, panel_x1, panel_y1, panel_x2, panel_y2, color_bgr=(20, 20, 20), alpha=0.28)

    # Başlık (Beyaz, büyük harf)
    title = "PARK HALINDE"
    cv2.putText(
        img,
        title,
        (panel_x1 + 14, panel_y1 + 34),
    ...
    # Satırlar alanı
    row_top = panel_y1 + 52
    row_left = panel_x1 + 12
    row_right = panel_x2 - 12
    row_h = 30
    row_gap = 8

    # Önce VIP varsa onu en üste al, sonra diğerlerini sırala
    parked_ids = sorted(set(current_frame_parked))
    ordered_ids = []
    if VIP_ID in parked_ids:
        ordered_ids.append(VIP_ID)
    for pid in parked_ids:
        if pid != VIP_ID:
            ordered_ids.append(pid)

    # Maks satır sayısı
    max_rows = int((panel_y2 - 14 - row_top) / float(row_h + row_gap))
    ordered_ids = ordered_ids[:max_rows]

    now_wall = time.time()

    for i, v_id in enumerate(ordered_ids):
        y1 = row_top + i * (row_h + row_gap)
        y2 = y1 + row_h

        # Satır arka planı: panelden biraz daha koyu
        draw_alpha_rect(img, row_left, y1, row_right, y2, color_bgr=(0, 0, 0), alpha=0.35)

        # Araç ikonu
        icon_x = row_left + 8
        icon_y = y1 + 5
        draw_car_icon(img, icon_x, icon_y, scale=0.85, color=(245, 245, 245))

        # Kayıt bilgileri
        rec = GLOBAL_PARKED_REGISTRY.get(v_id, {})
        plate = rec.get("plate", "[...]")
        st = rec.get("start_time")
        if st is None:
            elapsed_s = 0
        else:
            elapsed_s = int(now_wall - float(st))

        elapsed_txt = format_elapsed(elapsed_s)

        # Metin: | ID no | plaka | süre |
        # (ikon zaten var -> ikon sonrası başlat)
        text_x = icon_x + 34
        text_y = y1 + 21

        # Hepsini beyaz yazalım (senin isteğin)
        line_txt = f"| ID {v_id} | {plate} | {elapsed_txt} |"
        cv2.putText(
            img,
            line_txt,
            (text_x, text_y),
        ...
        # Yeşil durum noktası (en sağda)
        dot_r = 6
        dot_cx = row_right - 14
        dot_cy = y1 + int(row_h / 2)
        cv2.circle(img, (dot_cx, dot_cy), dot_r, (0, 255, 0), -1, cv2.LINE_AA)
        cv2.circle(img, (dot_cx, dot_cy), dot_r, (0, 0, 0), 1, cv2.LINE_AA)
